...
// Configuración de la API para consultar el registro civil de matrimonio
const apiUrl = 'https://adc-api.notaria62bogota.com/soap-signo/consultar-registro-matrimonio/';

// Función para convertir texto a audio en formato .slin
async function generarAudio(estado, uniqueId) {
    const audioPath = `/var/lib/asterisk/sounds/estado-rcm-${uniqueId}.slin`; // Ruta del archivo de audio
    ...
    try {
        ...
        const tempWav = `/tmp/estado-rcm-${uniqueId}.wav`;
...
async function consultar(param, uniqueId) {
    try {
       ...
       switch (estado) {
            case 'No encontrado':
                await generarAudio(`No se encontró el registro civil de Matrimonio con el número de documento ${param}, le recordamos que solo podemos expedir Registro civil de matrimonio que reposen en esta Notaría, si desea más información, presione dos para ser atentido por nuestro departamento de registro civil.`, uniqueId);
                break;
            case 'Activo':
                await generarAudio(`El registro civil de Matrimonio con el número de documento ${param} se encuentra activo, Para solicitar una copia de su registro civil, envíe un correo electrónico a: registro punto civil arroba notaría sesenta y dos bogotá punto com, escriba en el asunto del correo el tipo de registro que necesita y su número de documento. o si prefiere presione dos para ser atentido por nuestro departamento de registro civil.`, uniqueId);
                break;
            case 'Anulado':
                await generarAudio(`El registro civil de Matrimonio con el número de documento ${param} se encuentra anulado, si desea más información, presione dos para ser atentido por nuestro departamento de registro civil.`, uniqueId);
                break;
        }
